#!/usr/bin/env bash
# ============================================================================
# mission002-proof-matrix.sh — Mission 002 cross-domain proof (Media + Revenue).
# ============================================================================
# Proves Media/G-Star services execute on the SAME Runtime as Mission 001
# without a Revenue-specific redesign.
#
# Required env: MIGRATION_DATABASE_URL, DATABASE_URL
# Optional: PORT (8092), DATABASE_SSL (false), GIT_SHA (local)
# ============================================================================
set -euo pipefail
cd "$(dirname "$0")/.."
ROOT="$(pwd)"
...
echo "[proof-m002] build"
npm run build >/dev/null 2>&1
mkdir -p dist/sql
cp sql/grants.sql dist/sql/grants.sql 2>/dev/null || true

echo "[proof-m002] migrate (includes Mission 001 + 002 catalog seeds)"
...
LOG="$ROOT/.proof-m002-runtime.log"
echo "[proof-m002] start Runtime"
...
wait_ready "$LOG"
echo "[proof-m002] Runtime ready"

echo "[proof-m002] PASS A/B/C/X"
PROOF_MODE=full AION_RUNTIME_URL="$AION_RUNTIME_URL" node dist/mission002-proof-matrix.js

echo "[proof-m002] PASS — Mission 002 cross-domain matrix green"
