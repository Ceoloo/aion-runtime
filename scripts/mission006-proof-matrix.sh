#!/usr/bin/env bash
# ============================================================================
# mission006-proof-matrix.sh — Mission 006 PASS A/B/C against live Runtime.
# ============================================================================
# Sequence:
#   migrate → boot Runtime → PASS A (economics + missions) →
#   PASS B (failed executions + pending approvals) →
#   PASS C (cross-tenant DENY + tenant header required)
#
# Required env: MIGRATION_DATABASE_URL, DATABASE_URL
# Optional: PORT (8096), DATABASE_SSL (false), GIT_SHA (local)
# ============================================================================
set -euo pipefail
cd "$(dirname "$0")/.."
ROOT="$(pwd)"
...
LOG="$ROOT/.proof-m006-runtime.log"

echo "[proof-m006] start Runtime"
...
wait_ready "$LOG"
echo "[proof-m006] Runtime ready"

echo "[proof-m006] PASS A/B/C"
AION_RUNTIME_URL="$AION_RUNTIME_URL" node dist/mission006-proof-matrix.js

echo "[proof-m006] PASS — Mission 006 matrix A/B/C green"
