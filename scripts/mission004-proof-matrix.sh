#!/usr/bin/env bash
# ============================================================================
# mission004-proof-matrix.sh — Mission 004 PASS A/B/C against live Runtime.
# ============================================================================
# Sequence:
#   migrate → boot Runtime → PASS A (multi-step lineage) →
#   PASS B (gated resume same root) → PASS C (deny mid-plan)
#
# Required env: MIGRATION_DATABASE_URL, DATABASE_URL
# Optional: PORT (8094), DATABASE_SSL (false), GIT_SHA (local)
# ============================================================================
set -euo pipefail
cd "$(dirname "$0")/.."
ROOT="$(pwd)"
...
echo "[proof-m004] build"
npm run build >/dev/null 2>&1
mkdir -p dist/sql
cp sql/grants.sql dist/sql/grants.sql 2>/dev/null || true

echo "[proof-m004] migrate (includes workflows + lineage)"
...
LOG="$ROOT/.proof-m004-runtime.log"

echo "[proof-m004] start Runtime"
...
wait_ready "$LOG"
echo "[proof-m004] Runtime ready"

echo "[proof-m004] PASS A/B/C"
AION_RUNTIME_URL="$AION_RUNTIME_URL" node dist/mission004-proof-matrix.js

echo "[proof-m004] PASS — Mission 004 matrix A/B/C green"
